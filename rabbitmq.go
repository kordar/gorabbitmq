...
type Receiver interface {
	...
	QueueName() string
	RoutingKey() string
	QueueDeclareParam() (durable, autoDelete, exclusive, noWait bool, args amqp.Table)
	QueueBindParam() (noWait bool, args amqp.Table)
	QosParam() (prefetchCount, prefetchSize int, global bool)
	ConsumeParam() (consumer string, autoAck, exclusive, noLocal, noWait bool, args amqp.Table)
}
...
func (mq *RabbitMQ) listen(receiver Receiver) {
	...
	// 申明Queue
	qdurable, qautoDelete, qexclusive, qnowait, qargs := receiver.QueueDeclareParam()
	_, err := mq.channel.QueueDeclare(queueName, qdurable, qautoDelete, qexclusive, qnowait, qargs)

	if nil != err {
		// 当队列初始化失败的时候，需要告诉这个接收者相应的错误
		receiver.OnError(fmt.Errorf("initialize the queue [%s] failed, %s", queueName, err.Error()))
	}

	bnoWait, bargs := receiver.QueueBindParam()
	// 将Queue绑定到Exchange上去
	err = mq.channel.QueueBind(queueName, routingKey, mq.exchangeName, bnoWait, bargs)

	if nil != err {
		receiver.OnError(fmt.Errorf("binding queue \"%s\" use \"%s\" to the exchange error, %s", queueName, routingKey, err.Error()))
	}
	...
	consumer, ack, exclusive, local, wait, args := receiver.ConsumeParam()
	msgs, err := mq.channel.Consume(queueName, consumer, ack, exclusive, local, wait, args)

	if nil != err {
		receiver.OnError(fmt.Errorf("get the queue \"%s\" for the comsume channel error, %s", queueName, err.Error()))
	}

	logger.Infof("the queue \"%s\" is listening for channel msg.", queueName)
	// 使用callback消费数据
	for msg := range msgs {
		// 当接收者消息处理失败的时候，
...
func NewDefaultRabbitMQ(exchangeName string, exchangeType string) *RabbitMQ {
	return NewRabbitMQ(exchangeName, exchangeType, false, false, false, false, nil)
}

func NewRabbitMQ(exchangeName string,
	exchangeType string,
	exchangeDurable bool,
	exchangeAutoDelete bool,
	exchangeInternal bool,
	exchangeNoWait bool,
	exchangeArgs amqp.Table) *RabbitMQ {
	return &RabbitMQ{
		wg:                 sync.WaitGroup{},
